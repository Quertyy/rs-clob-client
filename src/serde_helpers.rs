...
/// A `serde_as` type that deserializes strings or numbers as `Decimal`.
///
/// Use with `#[serde_as(as = "DecimalFromAny")]` for `Decimal` fields
/// or `#[serde_as(as = "Option<DecimalFromAny>")]` for `Option<Decimal>`.
#[cfg(any(feature = "clob", feature = "data", feature = "gamma"))]
...
impl<'de> serde_with::DeserializeAs<'de, rust_decimal::Decimal> for DecimalFromAny {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct DecimalOrNumberVisitor;

        impl Visitor<'_> for DecimalOrNumberVisitor {
            type Value = Decimal;
        ...
        deserializer.deserialize_any(DecimalOrNumberVisitor)
    }
}
...
mod tests {
    ...
    mod decimal_from_any_tests {
        ...
        #[derive(Debug, Deserialize, PartialEq, serde::Serialize)]
        struct DecimalFromAnyStruct {
            #[serde(with = "serde_with::As::<DecimalFromAny>")]
            value: Decimal,
        }

        #[test]
        fn decimal_from_string() {
            let json = serde_json::json!({ "value": "123.456" });
            let result: DecimalFromAnyStruct =
                serde_json::from_value(json).expect("deserialization failed");
            assert_eq!(result.value, dec!(123.456));
        }
        ...
        #[test]
        fn decimal_from_negative_integer() {
            let json = serde_json::json!({ "value": -100 });
            let result: DecimalFromAnyStruct =
                serde_json::from_value(json).expect("deserialization failed");
        ...
        fn decimal_from_float() {
            let json = serde_json::json!({ "value": 3.14 });
            let result: DecimalFromAnyStruct =
                serde_json::from_value(json).expect("deserialization failed");
            // Float conversion may have precision loss, check approximate
            assert!((result.value - dec!(3.14)).abs() < dec!(0.0001));
        }

        #[test]
        fn decimal_from_zero_string() {
        ...
        fn decimal_serialize() {
            let obj = DecimalFromAnyStruct { value: dec!(99.99) };
            let json = serde_json::to_value(&obj).expect("serialization failed");
            assert_eq!(json, serde_json::json!({ "value": "99.99" }));
        }
    }
