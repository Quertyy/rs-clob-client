...
use polymarket_client_sdk::{POLYGON, PRIVATE_KEY_VAR, PUSD, contract_config};
...
const TOKEN_TO_APPROVE: Address = PUSD;
...
async fn approve<P: alloy::providers::Provider>(
    pusd: &IERC20::IERC20Instance<P>,
    spender: Address,
    amount: U256,
) -> anyhow::Result<alloy::primitives::FixedBytes<32>> {
    let tx_hash = pusd.approve(spender, amount).send().await?.watch().await?;
    Ok(tx_hash)
}
